...
app._initializeWSS = function (server) {
    ...
    this._wss.on('connection', function (socket) {
        ...
        let key = query.key;
        let ip = socket.upgradeReq.socket.remoteAddress;
        ...
        try {
            ...
            if (['LEAVE', 'CANDIDATE', 'OFFER', 'ANSWER'].indexOf(message.type) !== -1) {
                ...
                if (socket.session.authenticated) {
                    switch (message.type) {
                        case "FRIEND_REQUEST":
                            UserManager.addFriendRequest(socket.session.user, message.payload.targetUser);
                            break;
                        case "ACCEPT_FRIEND_REQUEST":
                            console.log("ACCEPT_FRIEND_REQUEST")
                            UserManager.acceptFriendRequest(socket, message.payload.requestID);
                            break;
                        case "REJECT_FRIEND_REQUEST":
                            console.log("REJECT_FRIEND_REQUEST")
                            UserManager.rejectFriendRequest(socket, message.payload.requestID);
                            break;
                        default:
...
app._initializeHTTP = function () {
    ...
    this.get('/', function (req, res, next) {
        res.send(require('../peerjs-server/app.json'));
    });

    // Retrieve guaranteed random ID.
    this.get('/:key/id', function (req, res, next) {
        let id = self._generateClientId(req.params.key);
        if (req.session.authenticated) {
            req.session.user.peerID = id;
        }
    ...
    this.get('/:key/peers', function (req, res, next) {
        let key = req.params.key;
        console.log(Object.keys(self._clients));
